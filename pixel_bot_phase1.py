...
def main_menu():
    keyboard = [
        ...
        [InlineKeyboardButton("🤖 ASK BOT (NEW)", callback_data="ask_bot_new")]
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🏢 *Pixel Office Phase 1*\n\n"
        "🤖 กด 'ASK BOT (NEW)' เพื่อใช้ Multi-Agent Orchestrator\n"
        "📌 ระบบ Tasks และ Dossier ยังทำงานเหมือนเดิม",
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "ask_bot_new":
        context.user_data["chatting_with_bot"] = True
        context.user_data["chat_history"] = []
        await query.edit_message_text(
            "🤖 *Orchestrator Agent พร้อมทำงาน*\n\n"
            "พิมพ์คำถามเกี่ยวกับการลงทุนได้เลย\n"
            "(Phase 1: ใช้ General Agent ก่อน)\n"
            "พิมพ์ /cancel เพื่อออก",
            reply_markup=main_menu(), parse_mode="Markdown"
        )
    else:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Chat with Orchestrator (NEW)
    if context.user_data.get("chatting_with_bot"):
        progress = await update.message.reply_text("🧠 Orchestrator กำลังคิด...")
        history = context.user_data.get("chat_history", [])
        
        # เรียก Orchestrator
        answer = run_orchestrator(text, history)
        ...
        await update.message.reply_text("กรุณากดปุ่ม 'ASK BOT (NEW)' ในเมนู", reply_markup=main_menu())
...
def main():
    ...
    print("🤖 Pixel Office Phase 1: Orchestrator + Legacy UI started.")
    app.run_polling()
